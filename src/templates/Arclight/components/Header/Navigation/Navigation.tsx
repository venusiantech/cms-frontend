  ...
  return (
    ...
    >
      ...
      {menuItem.children?.length && (
        <ChevronDown className="ms-1 -me-1 size-4 text-neutral-400" aria-hidden="true" />
      )}
    </Link>
  )
}
...
const DropdownMenu = ({ menuItem }: { menuItem: TNavigationItem }) => {
  const renderMenuLink = (menuItem: TNavigationItem) => {
    return (
      ...
      >
        {menuItem.name}
        {menuItem.children?.length && <ChevronDown className="ml-2 h-4 w-4 text-neutral-500" aria-hidden="true" />}
      </Link>
    )
  }
